...
import { CollectionsManager, type Collection } from "@/components/portal/collections-manager";
    ...
    <div className="grid lg:grid-cols-[1fr_240px] gap-8 items-start max-w-4xl">
      ...
      <div className="space-y-10">
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div>
            <label className="block text-sm text-brand-ink mb-2">Foto de perfil</label>
          ...
          <div>
            <label className="block text-sm text-brand-ink mb-2">Username</label>
            <input
              required
              value={name}
          ...
          {items.length > 0 && (
            <div>
              <label className="block text-sm text-brand-ink mb-1">Qué marcas se ven, y en qué orden</label>
              <p className="text-xs text-brand-ink-soft mb-2">Así aparecen en tu vitrina, de arriba hacia abajo.</p>
              <div className="space-y-1.5">
                {items.map((it, i) => (
                  <div
                    key={it.id}
                    className={`flex items-center gap-3 rounded-xl border px-3 py-2 transition-opacity ${
                      it.visible ? "border-brand-line bg-brand-surface" : "border-brand-line bg-brand-surface opacity-50"
                    }`}
                  >
                    <span className="font-mono text-[10px] text-brand-ink-soft w-4 text-center shrink-0">{i + 1}</span>
