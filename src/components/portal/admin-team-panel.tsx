  ...
  [
    {
      ...
      label: "Propietario",
      description:
        "Acceso total. Es el único rol que puede editar Configuración de la plataforma y gestionar el Equipo (agregar, cambiar rol o eliminar personas).",
    },
    {
      role: "SUPPORT",
      label: "Soporte",
      description:
        "Atiende el día a día: revisa transacciones, mensajería y responde dudas de marcas y creadores.",
    },
    {
      role: "FINANCE",
      label: "Finanzas",
      description:
        "Sigue de cerca cobros a marcas, pagos a creadores, costos mensuales y reportes financieros.",
    },
    {
      role: "BRAND_APPROVER",
      label: "Aprobador de marcas",
      description:
        "Encargado de revisar y aprobar o rechazar el registro de nuevas marcas.",
    },
    {
      role: "CREATOR_APPROVER",
      label: "Aprobador de creadores",
      description:
        "Encargado de dar seguimiento a los creadores: hoy esto significa suspender o reactivar cuentas desde Creadores. Los creadores quedan activos automáticamente al registrarse — no hay una cola de aprobación separada por ahora.",
    },
  ];
  ...
  return (
    <div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[480px]">
            ...
            <tbody className="divide-y divide-brand-line">
              {admins.map((a) => (
                <tr key={a.id}>
                  <td className="px-5 py-3 text-brand-ink font-mono">
                    {a.email}
                  </td>
      ...
      <h2 className="font-display font-semibold text-brand-ink mb-4">
        Agregar a alguien de tu equipo
      </h2>
      <form onSubmit={handleAdd} className="space-y-4 max-w-sm">
        ...
        <p className="text-xs text-brand-ink-soft">
          Comunícale la contraseña temporal por fuera de la plataforma — no se
          envía por correo todavía.
        </p>
      </form>

      <h2 className="font-display font-semibold text-brand-ink mt-10 mb-4">
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-brand-line">
            {roleExplanation.map((r) => (
              <tr key={r.role}>
                <td className="px-5 py-3 text-brand-ink font-medium align-top">
                  {r.label}
                </td>
                <td className="px-5 py-3 text-brand-ink-soft align-top">
                  {r.description}
                </td>
              </tr>
            ))}
