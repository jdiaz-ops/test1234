...
export default async function AdminFacturacionPage() {
  ...
  return (
    <div>
      ...
      <h1 className="font-display text-2xl font-semibold text-brand-ink mb-2">Verificar pagos de marcas</h1>
      <p className="text-sm text-brand-ink-soft mb-8 max-w-lg">
        Las marcas pagan por transferencia directa (QR/Bre-B) y suben su comprobante — verifícalo aquí para
        reactivarlas. Configura las instrucciones de pago en Configuración.
      </p>

      <h2 className="font-display font-semibold text-brand-ink mb-4">Pendientes de verificar</h2>
      <div className="mb-10">
        <AdminBillingQueue
          items={pending.map((c) => ({
            id: c.id,
      ...
      <h2 className="font-display font-semibold text-brand-ink mb-4">Historial reciente</h2>
      {recent.length === 0 ? (
        <p className="text-sm text-brand-ink-soft">Todavía no hay cortes resueltos.</p>
      ) : (
            ...
            <tbody className="divide-y divide-brand-line">
              {recent.map((c) => (
                <tr key={c.id}>
                  ...
                  <td className="px-5 py-3 text-brand-ink-soft">{statusLabel[c.status]}</td>
                </tr>
              ))}
            </tbody>
          </table>
