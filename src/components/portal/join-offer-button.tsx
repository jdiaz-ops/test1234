...
export function JoinOfferButton({
    ...
    return (
      <div className="space-y-2">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => setEditing(false)} className="text-xs text-brand-ink-soft hover:underline">
            Cancelar
          </button>
        </div>
      </div>
  ...
  return (
    <button
      ...
      className="bg-brand-accent text-white rounded-full px-4 py-1.5 text-sm font-medium hover:opacity-90"
    >
      {joinMode === "OPEN" ? "Unirme" : "Solicitar unirme"}
    </button>
  );
}
