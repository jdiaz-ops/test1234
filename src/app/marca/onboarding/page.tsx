...
export default async function OnboardingPage() {
  ...
  const [{ steps, complete, completedCount, total }, summary, platformConfig] =
    await Promise.all([
    ...
    <div>
      ...
      <div className="rounded-2xl border-2 border-brand-accent bg-brand-accent-soft px-5 py-4 mb-6 max-w-2xl">
        <p className="font-display text-lg font-semibold text-brand-ink leading-snug">
          Tu marca solo se hace visible para los creadores de contenido cuando
          completes estos {total} pasos
          {profile.status !== "APPROVED" &&
            " y sea aprobada por nuestro equipo"}
          .
        </p>
      </div>
