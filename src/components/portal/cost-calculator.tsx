...
export function CostCalculator({
  ...
  const effectiveCommission = editableRates
    ...
    <div className="rounded-xl bg-brand-bg border border-brand-line overflow-hidden">
      <div className="flex items-center gap-2 bg-brand-accent-soft px-4 py-2 border-b border-brand-line">
        ...
        <span className="text-xs text-brand-ink">
          así se reparte una venta con tus números
        </span>
      </div>
      <div className="p-4 space-y-3">
        {editableTicket && (
          <div>
            <label className="block text-xs text-brand-ink-soft mb-1">
              Tu ticket promedio de compra
            </label>
            <CurrencyInput
              value={ticketRaw}
              onChange={setTicketRaw}
              placeholder="100.000"
            />
          </div>
        )}
        ...
        <div className="text-xs font-mono space-y-1.5">
          ...
          <div className="flex justify-between text-brand-ink-soft">
            <span>Tarifa Marcolini ({platformFeePercent}%)</span>
