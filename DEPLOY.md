...
| `EMAIL_FROM` | Opcional. Déjala vacía mientras no tengas un dominio propio verificado en Resend — los correos salen igual, desde la dirección de prueba de Resend, pero **solo le llegan a la cuenta de correo con la que te registraste en Resend** (sirve para probar tú mismo, no para usuarios reales todavía). Cuando verifiques tu dominio en Resend (Domains → Add Domain), ponla aquí: `Marcolini <no-reply@tu-dominio.com>`. |
