...
function CollectionEditor({
  ...
  const [selected, setSelected] = useState<Product[]>(
    collection?.items.map((i) => i.product) ?? [],
  );
  ...
  async function save() {
    ...
    const productsRes = await fetch(
      `/api/creador/colecciones/${collectionId}/productos`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      <div className="grid gap-6 @2xl:grid-cols-[260px_1fr]">
        <div>
          ...
          <p className="text-xs text-brand-ink-soft mb-1">
            Vista previa ({selected.length} producto
            {selected.length === 1 ? "" : "s"})
          </p>
          <div className="grid grid-cols-2 gap-2">
            {selected.map((p) => (
              <div
                key={p.id}
                className="rounded-xl border border-brand-line bg-brand-surface p-2 relative"
              >
                <button
                ...
                <p className="text-[10px] text-brand-ink-soft truncate">
                  {p.brand.companyName}
                </p>
                <p className="text-[11px] font-medium text-brand-ink truncate">
                  {p.name}
                </p>
              </div>
            ))}
          ...
          <div className="grid grid-cols-2 @sm:grid-cols-3 @lg:grid-cols-4 gap-2 max-h-72 overflow-y-auto">
                  ...
                  <span
                    className={`absolute top-1 right-1 w-5 h-5 rounded-full text-xs flex items-center justify-center ${
                      isSelected
                        ...
                        : "bg-white/90 border border-brand-line text-brand-ink-soft"
                    }`}
                  >
                    {isSelected ? "✓" : "+"}
                  </span>
            ...
            {results.length === 0 && (
              <p className="col-span-full text-xs text-brand-ink-soft py-4">
                No hay productos — únete a marcas y sincroniza su catálogo para
                poder agregar.
              </p>
            )}
          </div>
  ...
  return (
    <div className="rounded-2xl border border-brand-line bg-brand-surface p-5 sm:p-6 mb-8">
      ...
      <p className="text-sm text-brand-ink-soft mb-1">
        Agrupa productos por tema — se muestran en tu vitrina, debajo de tus
        marcas.
      </p>
      ...
      {collections.length > 0 && (
        <div className="space-y-2 mb-4">
          {collections.map((c, i) => (
            <div
              key={c.id}
              className="flex items-center gap-3 rounded-xl border border-brand-line bg-brand-bg px-3 py-2.5"
            >
              ...
              <div className="flex -space-x-2 shrink-0">
                {c.items.slice(0, 3).map((it) =>
                  it.product.imageUrl ? (
                    ...
                    <div
                      key={it.product.id}
                      className="w-8 h-8 rounded-lg bg-brand-accent-soft border-2 border-brand-surface"
                    />
                  ),
                )}
              </div>
      ...
      {editing === "new" && (
        <CollectionEditor collection={null} onDone={() => setEditing(null)} />
      )}
      {editingCollection && (
        <CollectionEditor
