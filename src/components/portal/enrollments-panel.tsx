...
function OverrideEditor({ enrollment, onDone }: { enrollment: Enrollment; onDone: () => void }) {
  const router = useRouter();
  const [commission, setCommission] = useState(
    enrollment.commissionPercentOverride ?? Number(enrollment.offer.defaultCommissionPercent)
  );
  const [discount, setDiscount] = useState(
    enrollment.discountPercentOverride ?? Number(enrollment.offer.defaultDiscountPercent)
  );
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button onClick={save} disabled={saving} className="text-xs text-brand-accent font-medium hover:underline">
        {saving ? "..." : "Guardar"}
      </button>
          ...
          {enrollments.map((e) => (
            <tr key={e.id}>
              <td className="px-5 py-3 text-brand-ink">
                ...
                {e.creator.city && <span className="text-brand-ink-soft"> · {e.creator.city}</span>}
              </td>
              ...
              <td className="px-5 py-3">
                {editingId === e.id ? (
                  ...
                  >
                    {e.commissionPercentOverride ?? Number(e.offer.defaultCommissionPercent)}% /{" "}
                    {e.discountPercentOverride ?? Number(e.offer.defaultDiscountPercent)}%
                  </button>
                )}
              </td>
              <td className="px-5 py-3 text-brand-ink-soft">{statusLabel[e.status]}</td>
              <td className="px-5 py-3">
